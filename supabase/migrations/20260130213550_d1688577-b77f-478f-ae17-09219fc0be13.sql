CREATE OR REPLACE FUNCTION public.create_order_secure(p_customer_name text, p_customer_email text, p_customer_phone text, p_delivery_address text, p_delivery_area_id uuid, p_delivery_speed delivery_speed, p_notes text, p_items jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Calculate priority fee from server (not client) - 19 kr for priority
